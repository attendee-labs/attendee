...
| `USE_SECURE_COOKIE_FOR_SIGNED_IN_GOOGLE_MEET_BOTS` | Boolean | `true` | Add the Secure flag to the Google Meet SSO sign-in session cookie. Set to `false` when the in-cluster Google Meet sign-in flow must use the cookie over HTTP. |
