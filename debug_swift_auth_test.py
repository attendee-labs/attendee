...
def test_auth_methods():
    ...
    # Test method 3: Try with username/password from OS_ variables
    username = os.getenv("OS_USERNAME")
    password = os.getenv("OS_PASSWORD") 
    project_name = os.getenv("OS_PROJECT_NAME")
    
    if username and password and project_name:
        print("\nTest 3: Username/password authentication (OS_ variables)")
        try:
            conn = client.Connection(
                authurl=os.getenv("OS_AUTH_URL"),
                auth_version="3",
    ...
    # Test method 4: Try with legacy SWIFT_ username/password if available
    ...
    if username and password and tenant:
        print("\nTest 4: Username/password authentication (SWIFT_ variables)")
        try:
            conn = client.Connection(
                authurl=os.getenv("OS_AUTH_URL"),
                user=username,
