...
1. **Metadata-First**: All custom data (transcript IDs, client info) passed via metadata from day one
2. **Webhook-Native**: Built-in webhook system for external service integration
3. **Storage-Agnostic**: Clean storage interface supporting Swift and future backends
4. **Transcript-Optimized**: Purpose-built for transcription workflows, not generic meeting bots
5. **Kubernetes-Native**: Designed for container orchestration from ground up

### **Implementation Requirements**
- **Modern Swift Storage**: Clean implementation without Django legacy patterns
- **Webhook System**: Reliable delivery with retry logic for transcription triggers
- **Metadata API**: Native support for passing transcript IDs and custom data
- **Clean Bot Controller**: Simplified recording logic without upstream complexity
- **Modern Deployment**: Cloud-native Helm charts and container patterns
