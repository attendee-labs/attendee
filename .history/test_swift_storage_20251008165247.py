...
def main():
    ...
    # Either application credentials or username/password
    app_cred_vars = ['SWIFT_AUTH_URL', 'SWIFT_APPLICATION_CREDENTIAL_ID', 'SWIFT_APPLICATION_CREDENTIAL_SECRET']
    user_pass_vars = ['SWIFT_AUTH_URL', 'SWIFT_USERNAME', 'SWIFT_PASSWORD', 'SWIFT_TENANT_NAME']
    
    # Check if we have application credentials
    app_cred_available = all(os.getenv(var) for var in app_cred_vars)
    # Check if we have username/password
    user_pass_available = all(os.getenv(var) for var in user_pass_vars)
    
    if not app_cred_available and not user_pass_available:
        print("Swift configuration missing. You need either:")
        print("\nOption 1 - Application Credentials (recommended):")
        for var in app_cred_vars:
            if not os.getenv(var):
                print(f"  - {var}")
        print("\nOption 2 - Username/Password:")
        for var in user_pass_vars:
            if not os.getenv(var):
                print(f"  - {var}")
        print("\nPlease set one of these credential sets and try again.")
