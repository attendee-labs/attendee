...
- Edit the `.env` file and configure storage (choose one option):
  ...
  **Option B: Use MinIO (local S3 alternative - no AWS account needed)**
...
- Start all the services:
  ...
  - **With MinIO**: `docker compose -f dev.docker-compose.yaml -f dev.docker-compose.local.yaml up`
...
- The confirmation link will be written to the server logs in the terminal where you ran `docker compose up`. Should look like `http://localhost:8000/accounts/confirm-email/<key>/`.
...
- (Optional) If using MinIO, you can access the MinIO Console at http://localhost:9001 (login: minioadmin/minioadmin) to view stored recordings.
