// Global variable to track meet_messages data channel
let meetMessagesDataChannel = null

function sendChatMessage(text) {
    try {
        console.error(`[Chat] Sending chat message: "${text}"`)
        ...
        if (
            ...
            console.error('[Chat]  meet_messages channel not available')
            return false
        }

        // Get current user for message metadata
        const allUsers = Array.from(window.userManager.allUsersMap.values())
        ...
        if (!currentUser) {
            console.error('[Chat] Current user not found')
            return false
        }
        ...
        if (!window.__meetMessagesCounter) {
            window.__meetMessagesCounter = 1
        }
        const messageCounter = window.__meetMessagesCounter++
        ...
        const textWriter = window.protobuf.Writer.create()
        ...
        const dataWriter = window.protobuf.Writer.create()
        ...
        const wrapperWriter = window.protobuf.Writer.create()
        ...
        const contentWriter = window.protobuf.Writer.create()
        ...
        const innerWriter = window.protobuf.Writer.create()
        ...
        console.error('[Chat] Chat message sent successfully')
        return true
    } catch (e) {
...
class StyleManager {
    ...
    checkNeededInteractions() {
        ...
        const recordingDialog = document.querySelector(
            'div[aria-modal="true"][role="dialog"]',
        )

        if (
            recordingDialog &&
            (recordingDialog.textContent.includes(
                'This video call is being recorded',
            ) ||
                recordingDialog.textContent.includes(
                    'This video call is being transcribed',
                ) ||
            ...
            const joinNowButton = recordingDialog.querySelector(
            ...
            if (joinNowButton) {
                try {
                    ...
                    window.ws.sendJson({
                        type: 'UiInteraction',
                        message:
                            'Automatically accepted recording notification',
                    })
                } catch (error) {
                    window.ws.sendJson({
                        type: 'Error',
                        message:
                            'Error clicking button to accept recording notification',
                    })
                }
            } else {
                window.ws.sendJson({
                    type: 'Error',
                    message:
                        'Found recording dialog but could not find button to accept recording notification',
                })
            }
        }
        ...
        if (
            removedFromMeetingElement &&
            (removedFromMeetingElement.textContent.includes(
    ...
    startSilenceDetection() {
        ...
        const mixedSource = this.audioContext.createMediaStreamSource(
            destination.stream,
        )
            ...
            const transformStream = new TransformStream({
                async transform(frame, controller) {
                    ...
                    try {
                        ...
                        console.error(
                            'Error processing mixed audio frame:',
                            error,
                        )
                        frame.close()
                    }
        ...
        if (window.userManager.getCurrentUsersInMeeting().length > 1) {
            ...
            if (!deviceIdOfBot) {
                window.ws.sendJson({
                    type: 'Error',
                    message:
                        'In hideBotVideoElement, window.userManager.currentUserId was null.',
                })
                return
            }
            const botVideoElement = document.querySelector(
                `[data-participant-id="${deviceIdOfBot}"]`,
            )
            if (!botVideoElement) {
                window.ws.sendJson({
                    type: 'Error',
                    message:
                        'In hideBotVideoElement, no bot video element found.',
                })
                return
            }
            const botOtherOptionsButton = botVideoElement.querySelector(
            ...
            if (!botOtherOptionsButton) {
                window.ws.sendJson({
                    type: 'Error',
                    message:
                        'In hideBotVideoElement, no bot other options button found.',
                })
                return
            }
            ...
            for (let i = 0; i < numAttempts; i++) {
                ...
                botMinimizeButton = document.querySelector(
                    'li[aria-label="Minimize"]',
                )
                ...
                if (wasLastAttempt) {
                    window.ws.sendJson({
                        type: 'Error',
                        message:
                            'In hideBotVideoElement, no bot minimize button found after polling.',
                    })
                    return
                }
            }
            ...
            window.ws.sendJson({
                type: 'Error',
                message:
                    'In hideBotVideoElement, no other users in meeting. So unable to minimize bot video element.',
            })
        }
        ...
        for (let i = 0; i < numAttempts; i++) {
            ...
            if (wasLastAttempt) {
                window.ws.sendJson({
                    type: 'Error',
                    message:
                        'In hideBotVideoElement, no bot minimized element found after polling.',
                })
                return
            }
        }
    ...
    showAllOfGMeetUI() {
        ...
        console.log(
            'Restored all hidden elements to their original display values',
        )
    }
    ...
    unpinPinnedVideos() {
        const participantList = document.querySelector(
        ...
        const participantListItems = participantList.querySelectorAll(
            'div[role="listitem"]',
        )
        ...
        for (const participantListItem of participantListItems) {
            ...
            const pinnedElements = Array.from(
                participantListItem.querySelectorAll('i[aria-hidden="true"]'),
            ...
            const actionsButton = participantListItem.querySelector(
    ...
    async openParticipantList() {
        ...
        if (peopleButton) {
            ...
            for (let i = 0; i < numAttempts; i++) {
                ...
                if (wasLastAttempt) {
                    console.log(
                        'Failed to find participant list after',
                        numAttempts,
                        'attempts',
                    )
                    window.ws.sendJson({
                        type: 'Error',
                        message:
                            'Failed to open participant list in openParticipantList',
                    })
                }
            }
    ...
    async openChatPanel() {
        ...
        if (chatButton) {
            ...
            for (let i = 0; i < numAttempts; i++) {
                ...
                if (wasLastAttempt) {
                    console.log(
                        'Failed to find chat input after',
                        numAttempts,
                        'attempts',
                    )
                    window.ws.sendJson({
...
class VideoTrackManager {
    ...
    upsertVideoTrack(videoTrack, streamId, isScreenShare) {
        ...
        const trackInfo = {
            ...
            firstSeenAt: existingVideoTrack
                ? existingVideoTrack.firstSeenAt
                : Date.now(),
            streamId: streamId,
        }
    ...
    getTrackToSend() {
        const screenShareTracks = Array.from(this.videoTracks.values()).filter(
            (track) => track.isScreenShare,
        )
        const mostRecentlyCreatedScreenShareTrack = screenShareTracks.reduce(
            (max, track) => {
        ...
        const nonScreenShareTracks = Array.from(
            this.videoTracks.values(),
        ).filter((track) => !track.isScreenShare)
        const mostRecentlyCreatedNonScreenShareTrack =
            nonScreenShareTracks.reduce((max, track) => {
                return track.firstSeenAt > max.firstSeenAt ? track : max
...
class UserManager {
    ...
    getUserByFullName(fullName) {
        return Array.from(this.allUsersMap.values()).find(
    ...
    getCurrentUsersInMeeting() {
        return Array.from(this.currentUsersMap.values()).filter(
            (user) => user.status === this.MEETING_STATUS.IN_MEETING,
        )
    }

    getCurrentUsersInMeetingWhoAreScreenSharing() {
        return this.getCurrentUsersInMeeting().filter(
            (user) => user.parentDeviceId,
        )
    }
    ...
    newUsersListSynced(newUsersListRaw) {
        const newUsersList = newUsersListRaw.map((user) => {
            ...
            const { isCurrentUserString, ...userWithoutIsCurrentUserString } =
                user
        ...
        for (const user of newUsersList) {
            if (
                ...
                JSON.stringify(this.currentUsersMap.get(user.deviceId)) !==
                    JSON.stringify(user)
            ) {
        ...
        const newUsers = newUsersList.filter(
            (user) => !previousUserIds.has(user.deviceId),
        )
        ...
        const updatedUsers = Array.from(updatedUserIds).map((id) =>
        ...
        if (
            newUsersToSend.length > 0 ||
            removedUsersToSend.length > 0 ||
            updatedUsersToSend.length > 0
        ) {
            this.ws.sendJson({
                type: 'UsersUpdate',
  ...
  */
    ...
    sendJson(data) {
        ...
        try {
            ...
            new DataView(message.buffer).setInt32(
                0,
                WebSocketClient.MESSAGE_TYPES.JSON,
                true,
            )
    ...
    sendEncodedMP4Chunk(encodedMP4Data) {
        if (this.ws.readyState !== WebSocket.OPEN) {
            console.error(
                'WebSocket is not connected for video chunk send',
                this.ws.readyState,
            )
            return
        }
        ...
        try {
            ...
            headerView.setInt32(
                0,
                WebSocketClient.MESSAGE_TYPES.ENCODED_MP4_CHUNK,
                true,
            )
    ...
    sendPerParticipantAudio(participantId, audioData) {
        if (this.ws.readyState !== WebSocket.OPEN) {
            console.error(
                'WebSocket is not connected for per participant audio send',
                this.ws.readyState,
            )
            return
        }
        ...
        try {
            ...
            const message = new Uint8Array(
                4 + 1 + participantIdBytes.length + audioData.buffer.byteLength,
            )
            ...
            dataView.setInt32(
                0,
                WebSocketClient.MESSAGE_TYPES.PER_PARTICIPANT_AUDIO,
                true,
            )
    ...
    sendMixedAudio(timestamp, audioData) {
        if (this.ws.readyState !== WebSocket.OPEN) {
            console.error(
                'WebSocket is not connected for audio send',
                this.ws.readyState,
            )
            return
        }
    ...
    sendVideo(timestamp, streamId, width, height, videoData) {
        if (this.ws.readyState !== WebSocket.OPEN) {
            console.error(
                'WebSocket is not connected for video send',
                this.ws.readyState,
            )
            return
        }
...
class RTCInterceptor {
    constructor(callbacks) {
        ...
        // Store callbacks
        const onPeerConnectionCreate =
            callbacks.onPeerConnectionCreate || (() => {})
        ...
        window.RTCPeerConnection = function (...args) {
            ...
            const originalCreateDataChannel =
                peerConnection.createDataChannel.bind(peerConnection)
...
if (window.initialData.sendPerParticipantAudio) {
    rtpReceiverInterceptor = new RTCRtpReceiverInterceptor(
        (receiver, result, ...args) => {
...
new FetchInterceptor(async (response) => {
    if (response.url === syncMeetingSpaceCollectionsUrl) {
        ...
        const userInfoListResponse =
            messageDecoders['UserInfoListResponse'](decodedData)
        const userInfoList =
            userInfoListResponse.userInfoListWrapperWrapper?.userInfoListWrapper
                ?.userInfoList || []
...
const handleCollectionEvent = (event) => {
    ...
    const deviceOutputInfoList =
        collectionEvent.body.userInfoListWrapperAndChatWrapperWrapper
            ?.deviceInfoWrapper?.deviceOutputInfoList
    ...
    const chatMessageWrapper =
        collectionEvent.body.userInfoListWrapperAndChatWrapperWrapper
            ?.userInfoListWrapperAndChatWrapper?.chatMessageWrapper
    ...
    const userInfoList =
        collectionEvent.body.userInfoListWrapperAndChatWrapperWrapper
            .userInfoListWrapperAndChatWrapper.userInfoListWrapper
            ?.userInfoList || []
    ...
    try {
        ...
        const isScreenShare = userManager
            ...
            .some(
                (user) =>
                    firstStreamId &&
                    userManager.getDeviceOutput(
                        user.deviceId,
                        DEVICE_OUTPUT_TYPE.VIDEO,
                    ).streamId === firstStreamId,
            )
        if (firstStreamId) {
            videoTrackManager.upsertVideoTrack(
                event.track,
                firstStreamId,
                isScreenShare,
            )
            ...
            async transform(frame, controller) {
                ...
                try {
                    ...
                    if (
                        firstStreamId &&
                        firstStreamId ===
                            videoTrackManager.getStreamIdToSendCached()
                    ) {
                        ...
                        */
                            ...
                            const currentTimeMicros = BigInt(
                ...
                try {
                    ...
                    if (
                        ...
                        JSON.stringify(currentFormat) !==
                            JSON.stringify(lastAudioFormat)
                    ) {
                    ...
                    const contributingSources =
                        receiverManager.getContributingSources(receiver)

                    const usersForContributingSourcesWithAudioLevel =
                        contributingSources
                            .map((source) => {
                                return {
                    ...
                    const userForContributingSourceWithLoudestAudio =
                        usersForContributingSourcesWithAudioLevel[0]?.user
                    ...
                    if (userForContributingSourceWithLoudestAudio) {
                        const firstUserId =
                            userForContributingSourceWithLoudestAudio?.deviceId
                        if (firstUserId) {
...
new RTCInterceptor({
    onPeerConnectionCreate: (peerConnection) => {
        ...
        peerConnection.addEventListener('datachannel', (event) => {
            ...
            if (event.channel.label === 'meet_messages') {
                meetMessagesDataChannel = event.channel
                console.error('[Chat] Chat channel ready')
            }
        })
        ...
        */
        ...
        setTimeout(() => {
            try {
                const meetMessagesChannel = peerConnection.createDataChannel(
                    'meet_messages',
                    { ordered: true },
                )
                ...
                console.error('[Chat] Failed to create chat channel:', e)
            }
        }, 100)
    },
    onDataChannelCreate: (dataChannel, peerConnection) => {
        ...
        if (dataChannel.label === 'meet_messages') {
            meetMessagesDataChannel = dataChannel
            console.error('[Chat] Chat channel ready (created by peer)')
            dataChannel.addEventListener('open', () => {
                meetMessagesDataChannel = dataChannel
                console.error('[Chat] Chat channel opened')
            })
            dataChannel.addEventListener('close', () => {
                if (meetMessagesDataChannel === dataChannel) {
                    meetMessagesDataChannel = null
                }
            })
        }
...
async function turnOnCamera() {
    ...
    for (let i = 0; i < numAttempts; i++) {
        ...
        window.ws.sendJson({
            type: 'Error',
            message:
                'Camera button not found in turnOnCamera, but will try again',
        })
        await new Promise((resolve) => setTimeout(resolve, 100))
    }
...
function turnOnMicAndScreenshare() {
    // Click microphone button to turn it on
    const microphoneButton = document.querySelector(
        `button[aria-label="${turnOnMicArialLabel}"]`,
    )
    if (microphoneButton) {
        console.log('Clicking the microphone button to turn it on')
        microphoneButton.click()
    ...
    const screenshareButton =
        document.querySelector(
            `button[aria-label="${turnOnScreenshareArialLabel}"]`,
        ) ||
        document.querySelector(
            `div[aria-label="${turnOnScreenshareArialLabel}"]`,
        )
    ...
    const botOutputMediaStreamSource =
        window.botOutputManager.audioContextForBotOutput.createMediaStreamSource(
            window.botOutputManager.botOutputMediaStream,
        )
...
function turnOffMicAndScreenshare() {
    // Click microphone button to turn it off
    const microphoneButton = document.querySelector(
        `button[aria-label="${turnOffMicArialLabel}"]`,
    )
    if (microphoneButton) {
        console.log('Clicking the microphone button to turn it off')
        microphoneButton.click()
...
function turnOffScreenshare() {
    ...
    const stopPresentingSpan = document.querySelector(
        'button span.UywwFc-vQzf8d[jsname="V67aGc"]',
    )
