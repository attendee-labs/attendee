...
def generate_aggregated_utterances(recording):
    utterances_sorted = sorted(recording.utterances.filter(async_transcription=recording.async_transcriptions.last()).all(), key=lambda x: x.timestamp_ms)
...
def generate_failed_utterance_json_for_bot_detail_view(recording):
    failed_utterances = recording.utterances.filter(async_transcription=recording.async_transcriptions.last()).filter(failure_data__isnull=False).order_by("timestamp_ms")[:10]
