...
(function () {
  if (window.__find_chat_candidates) return; // don't redefine
  ...
  function getFiberFromDOMNode(node) {
      ...
      function scan(value, localPath, depth) {
        ...
        if (Array.isArray(value)) {
          ...
          if (typeof first === "object" && looksLikeChatMessage(first, opts.sentinel)) {
            // Scan through existing out and only push if the raw contains a clientMessageId that is not already in the out array
    ...
    window.__find_chat_candidates = function (options) {
        const opts = options || {};
        ...
        // e.g. window.__find_chat_candidates({ containerNode: chatDiv, sentinel: "Message" })
